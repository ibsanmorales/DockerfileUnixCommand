...
RUN  apt-get -y update && \
     DEBIAN_FRONTEND=noninteractive apt-get install -yq apt-utils nano vim unzip git curl csvkit
...
RUN  curl -O  http://datosabiertos.salud.gob.mx/gobmx/salud/datos_abiertos/datos_abiertos_covid19.zip && \
     ...
     #INTUBADOS
     csvcut -c 3,6,10,13,14,16,17,25,28,34 210418COVID19MEXICO.csv | csvgrep -i -c 3 -m "9999-99-99" | csvgrep -i -c 4 -m "99" | csvgrep -c 1 -m "1" | csvgrep -c 10 -m "1" | csvgrep -c 5 -m "1" | csvcut -c 2,3,4,6,8,9 > fecha_defunciones_intubado_privado_limpia.csv && \
